...
#define DISK_SIZE       10000000 /* The backing file size is a constant 10MB */
#define FILE_SIZE       0xFFFFFFEED6918000           /*Maximum file size = 2^64 bytes*/

int FAT[DISK_SIZE/BLOCK_SIZE]; /*FAT table with one entry per block*/
...
#define BACKING_FILE_SIZE 10000000 /* The backing file size is a constant 10MB */
 ...
 */
#define DIRENT_LENGTH   64 // Directories should now be variable length
 ...
 */
typedef struct {
    size_t              block;        /* Number of the block in the system */
    size_t              size;           /* Size of the file */
    unsigned char       type;           /* Entry type (see below) */
    unsigned char       namelen;        /* Length of name */
    char                name[NAME_LENGTH];  /* Directory name */
}
                        fat_dirent;
 ...
 */
#define LAST_BLOCK(x)           (x+1) //build sample for loop to simulate walking through file

/*
 * Macros to convert a byte offset to a block in a file.
 */
// define OFFSET_TO_BLOCK(dirent, x) \                              (FILE_NO_TO_BLOCK(dirent->file_no) + (x) \                   / superblock.s.block_size)
#define OFFSET_IN_BLOCK(x)      ((x) % superblock.s.block_size)

/*
 * Number of directory entries stored in a block.
 */
#define DIRENTS_PER_BLOCK       (superblock.s.block_size / superblock.s.block_size)

/*
 * Number of blocks in FAT.
 */
#define BLOCKS_PER_FAT       (DISK_SIZE/BLOCK_SIZE+sizeof(int)-1)/sizeof(int)

/*
 * Scans the FAT for an empty slot
 */
static int scan_FAT()
{   int i = 0;
    for (i=0; i<(DISK_SIZE/BLOCK_SIZE); i++){
        if (FAT[i] == -1)
            return i;
    }
    return 0; //Return the superblock to indicate error
}
...
static void* fuse_fat_init(struct fuse_conn_info *conn, struct fuse_config *cfg)
{
    size_t              size;
    ...
    FAT[0] = 0; //For the superblock 

    FAT[1] = 0; //For the root directory

    FAT[2] = 0; //For the FAT itself
    // Set all entries in the FAT to -1 to mark that they are free
    
    int i;
    for (i = 2 + BLOCKS_PER_FAT ; i < DISK_SIZE/BLOCK_SIZE; i++){
        FAT[i] = -1;
    }
    ...
    //dirbuf[0].file_no = 1;
    dirbuf[0].block = 1;
    dirbuf[0].size = DIR_SIZE * superblock.s.block_size;
    dirbuf[0].namelen = 1;
    ...
    // dirbuf[1].file_no = 1;
    dirbuf[1].block = 1;
    dirbuf[1].size = DIR_SIZE * superblock.s.block_size;
    dirbuf[1].namelen = 2;
    ...
    for (cp = component_start = path;  *cp != '\0';  cp++) {
        if (*cp == '/') {
            if (cp != component_start) {
                ...
                dirent = lookup_component(dirent->block,
                  component_start, cp);
...
static int fuse_fat_getattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi)
{
    fat_dirent*      dirent;
...
// static int fuse_fat_fgetattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi)
// {
...
static int fuse_fat_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi, enum fuse_readdir_flags flags)
{
    block_t             block;
    fat_dirent*      dirent;
    ...
    if (offset != 0){
        skip_blocks = (int)(offset/DIRENT_LENGTH);
        int i = 0;
        for (i = 0; i < skip_blocks; i++){
            block = FAT[block];
        }
        block_offset = offset%DIRENT_LENGTH;
    }

    while (FAT[block]!= 0) {
        fetch_dirblock(block);
        for (dirent = dirbuf+block_offset;  dirent < dirend;  dirent++) {
            if (dirent->type != TYPE_EMPTY) {
                memcpy(name, dirent->name, dirent->namelen);
                name[dirent->namelen] = '\0';
                offset++;
                if (filler(buf, name, NULL, offset, 0))
                    return 0;
            }
        }
        block = FAT[block];
...
static int fuse_fat_mknod(const char *path, mode_t mode, dev_t rdev)
{
    ...
    block = parent_block = dirent->block;
    /*
     * Find an empty slot.
     */
    while (FAT[block] != 0) {
...
/*
 * NEEDSWORK: This shares a lot of code with fuse_fat_mknod.  The
...
static int fuse_fat_mkdir(const char *path, mode_t mode)
{
     ...
     */
    dirent = find_dirent(path, 1);
    parent_file_block = dirent->block;
    parent_size = dirent->size;
 
    block = parent_file_block;
...
doublebreak:
    ...
    dirblock = dirent->block;
    memset(dirbuf, 0, superblock.s.block_size); //Zeros the rest of the dirblock
    dirbuf[0].type = TYPE_DIR;
    dirbuf[0].block = dirent->block;
    dirbuf[0].size = 0;
    dirbuf[0].namelen = 1;
    memcpy(dirbuf[0].name, ".", 1); //Setting up the current info
    dirbuf[1].type = TYPE_DIR;
    dirbuf[1].block = parent_file_block;
    dirbuf[1].size = parent_size;
    dirbuf[1].namelen = 2;
    memcpy(dirbuf[1].name, "..", 2); //Setting up the parent info
    flush_dirblock();
...
static int fuse_fat_rename(const char *from, const char *to, unsigned int flags)
{
    /*
...
static int fuse_fat_truncate(const char *path, off_t size, struct fuse_file_info *fi)
{
    block_t             block;
  ...
  off_t offset, struct fuse_file_info *fi)
{
    ...
    for (bytes_written = 0;  size > 0;  block++, offset = 0) {
        if (offset != 0
          ||  (size < superblock.s.block_size
            &&  offset + size < dirent->size))
        ...
        if (offset > dirent->size) {//How to go past the end of the file?
            fuse_fat_truncate(path, offset+size, fi);
        }
...
static int fuse_fat_symlink(const char* to, const char* from) {
    fat_dirent*      from_dirent;
    fat_dirent*      to_dirent;

    from_dirent = find_dirent(from, 0);
    to_dirent = find_dirent(to, 0);
    if (to_dirent != NULL)
        return -EEXIST;
    if (from_dirent->type == TYPE_SYM){
        fuse_fat_create(to, O_RDWR, NULL);
        // to_buf = to;
        fuse_fat_write(from, to, strlen(to), 0, NULL);
    }
...
static int fuse_fat_readlink(const char* path, char* buf, size_t size)
{
    ...
    dirent = find_dirent(path, 0);
    if (dirent->type == TYPE_SYM)
        fuse_fat_read(path, buf, size, 0, NULL);
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    backing_file_fd = open("/home/ktsai/courses/cs137/FAT/fat_disk", O_RDWR | O_CREAT, 0600);
